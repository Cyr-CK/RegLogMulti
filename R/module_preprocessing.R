...
module_preprocessing_server <- function(id, data , target_variable, explanatory_variables) {
  moduleServer(id, function(input, output, session) {
    ...
    observeEvent(input$one_hot_encode_btn, {
      ...
      qualitative_cols <- qualitative_cols[!qualitative_cols %in% target_variable()]
    ...
    # Retourner les données finales et les nouveau noms des variables explicatives
    return(list(
      data = reactive(processed_data),
      explanatory_variables = reactive(names(processed_data()))
    ))
  })
}